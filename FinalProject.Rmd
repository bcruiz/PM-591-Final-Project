...
```{r Train Performance, echo=FALSE}
...
```{r, echo=FALSE}
# pred_NIS_test <- factor(predict(NIS_glm2, newdata = test, type = 'response')> 0.5) #
# 
...
```{r Fixed Train Performance, echo=FALSE}
...
# 
# roc_empirical <- 
#   ROCit::rocit(score = NIS_glm_fix$fitted.values, class = NIS_glm_fix$y)
# plot(roc_empirical, YIndex = F)
# summary(roc_empirical)
# ROCit::ciAUC(roc_empirical)
```

# Predicting Death within Hospital Stay
...
ggplot(data = NIS_simple, aes(x = DIED, y = AGE, fill = DIED))+
  geom_boxplot()+
  labs(title = 'Age and Outcome', x = 'Outcome', y = 'Age', fill = 'Outcome')+
  theme_bw()

ggplot(data = NIS_simple%>% filter(DIED == 'Died'), aes(x = AGE, fill = DIED))+
  geom_histogram(bins = 45)+
  labs(title = 'Death Frequencies by Age', x = 'Age', y = 'Frequency', fill = 'Outcome')+
  theme_bw()
...
plot(roc_glm, lwd = 1, col = 'black')
plot(roc_test_glm, lwd = 1, col = 'red', add = TRUE)
legend('bottomright', legend = c('Train', 'Test'), col = c('black', 'red'), lty = 1, cex = 0.8)
...
legend(0,0.4, legend = c('Train', 'Test'), col = c('black', 'red'), lty = 1:1, cex = 0.8)
```

### Balanced Random Forest (Even Better)
...
plot(roc_train)
plot(roc_test, col = 'red', add =TRUE)
legend(0,0.4, legend = c('Train', 'Test'), col = c('black', 'red'), lty = 1)
...
NIS_boost <- gbm(1*(DIED=='Died') ~ ., data = NIS_simple[train,],
                 distribution = 'bernoulli', n.trees = 8000,
                 interaction.depth = 1,
                 shrinkage = 0.01,
...
plot(roc_train, lwd = 3, cex.lab = 1.2, cex.axis = 1.2)
plot(roc_test, lwd = 3, col = 'red', add = TRUE)
legend(x = 'bottomright', legend = c('Train', 'Test'), lwd = 3, col = c('black', 'red'), cex = 1.2)
```
