...
```{r}
NIS_test <- NIS_simple%>%
  filter(DIED == 'Not Dead')

sel <- sample(nrow(NIS_test), 3371)
NIS_non <- NIS_test[sel,]

NIS_dead <- NIS_simple%>%
  filter(DIED == 'Died')

NIS_smpsize <- rbind(NIS_non, NIS_dead)

table(NIS_smpsize$DIED)
```

```{r}
NIS_tsk <- makeClassifTask(id = "Mortality within hospital",
                           data = NIS_smpsize, target = 'DIED')
...
train <- split$train.inds[[1]]
test <- split$test.inds[[1]]
table(NIS_smpsize$DIED[train])
```

```{r}
NIS_boost <- gbm(1*(DIED=='Died') ~ ., data = NIS_smpsize[train,],
